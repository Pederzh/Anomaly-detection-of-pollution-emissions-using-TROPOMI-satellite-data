...
fileU = pd.read_csv(
    "./data/" + location_name + "/wind/wind-20190101_20211007-10m_" + components[0] + "-500.csv",
    thousands=',')
fileV = pd.read_csv(
    "./data/" + location_name + "/wind/wind-20190101_20211007-10m_" + components[1] + "-500.csv",
    thousands=',')
...
date_first = date.replace(year=2019, month=1, day=1, hour=0, minute=0, second=0, microsecond=0)
...
if lenTimes > 1:
    for i, angle, intensity in zip(df.index, df['angle'], df['intensity']):
        ...
        val[()] = (round(angle, 1), round(intensity, 1))
        subMatrix = np.full((endX - startX, endY - startY), val, dtype=object)
    ...
    for i, angle, intensity in zip(df.index, df['angle'], df['intensity']):
        ...
        if i % locNum == 0:
            if i != 0:
                wind[date_start.strftime("%Y-%m-%d")] = wind[date_start.strftime("%Y-%m-%d")].tolist()  # to print on the file the matrixes TO FIX
            ...
            #wind[date_start.strftime("%Y-%m-%d")] = []
            date_start = date_start.replace(hour=times[timeIndex])
            wind[date_start.strftime("%Y-%m-%d")] = np.copy(windMatrix)
            timeIndex += 1
        ...
        wind[date_start.strftime("%Y-%m-%d")][startX:endX, startY:endY] = np.copy(subMatrix)

print(wind)
info_name_write = "wind-matrix_" + location_name.strip()
file_name_write = info_name_write + date_first.strftime("%Y-%m-%d") + "_" + date_end.strftime("%Y-%m-%d") + "-600"
