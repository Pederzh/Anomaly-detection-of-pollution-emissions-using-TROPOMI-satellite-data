...
for l in range(len(location_names)):
    ...
    for p in range(len(product_types)):
        directory_path = "./Data/" + location_name + "/" + product_types[p] + "/Statistics/"
        with open(directory_path + "days.json") as json_file:
            days = json.load(json_file)
        product_days[product_types[p]] = days
    ...
    for q in range(len(qualities)):
        ...
        for day_counter in range(int((date_end - date_start).days)):
            ...
            for i in range(len(product_types)):
                data = {"date": date_from_str,
                        ...
                        "value": product_days[product_types[i]][date_from_str]["statistics"]["mean"][quality]}
        ...
        directory_path_save = "./data/" + location_name + "/ALL/charts"
        Path(directory_path).mkdir(parents=True, exist_ok=True)
        file_name = location_name.strip() + "_" + str(product_types) + "_" + qualities_text[q] + "_" + date_start.strftime(
